...
// Term Registrations - with outbox event for finance invoice generation
func (s *Service) RegisterStudentForTerm(ctx context.Context, tr *domain.TermRegistration) error {
	...
	// Get program ID from student
	var student domain.Student
	if err := s.db.First(&student, tr.StudentID).Error; err != nil {
		return apperrors.NotFound("student")
	}

	// Get current semester from batch
	var semesterNo int
	if tr.BatchID > 0 {
		var batch domain.Batch
		if err := s.db.First(&batch, tr.BatchID).Error; err == nil {
			semesterNo = (time.Now().Year()-batch.AdmissionYear)*2 + 1
		}
	}
	...
	txErr := s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.outbox.WriteEvent(tx, "TermRegistration", fmt.Sprintf("%d", tr.ID),
			eventbus.EventTermRegistered,
			eventbus.TermRegisteredPayload{
				StudentID:      tr.StudentID,
				AcademicTermID: tr.AcademicTermID,
				BatchID:        tr.BatchID,
...
// Course Registrations - with outbox event
func (s *Service) RegisterStudentForCourse(ctx context.Context, cr *domain.CourseRegistration) error {
	...
	txErr := s.db.Transaction(func(tx *gorm.DB) error {
		...
		return s.outbox.WriteEvent(tx, "CourseRegistration", fmt.Sprintf("%d", cr.ID),
			eventbus.EventCourseRegistered,
			map[string]interface{}{
				"registration_id": cr.ID,
				"student_id":      cr.StudentID,
				"offering_id":     cr.OfferingID,
				"is_elective":     cr.IsElective,
			},
		)
	})
	return txErr
